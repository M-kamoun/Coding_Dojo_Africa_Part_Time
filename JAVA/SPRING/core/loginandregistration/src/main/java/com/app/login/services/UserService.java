	...
	private UserRepository userRepo;
	 ...
	 public User register(User newUser, BindingResult result) {
		...
		if(newUser.getBirthday() != null) {
			...
			if(age<10) {
				result.rejectValue("birthday","error.birthday", "You must be over 10 years old to register");
			}
		}
		
		if(newUser.getLanguages()==null || newUser.getLanguages().size()<2 ) {
			result.rejectValue("languages","error.languages", "You must select at least 2 languages!");
		}
